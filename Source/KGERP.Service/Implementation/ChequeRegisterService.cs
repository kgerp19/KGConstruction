    ...
    public class ChequeRegisterService : IChequeRegisterService
    {
        ...
        public async Task<List<ChequeBookModel>> GetChequeBookList(int companyId)
        {
            List<ChequeBookModel> sendData = await (from t1 in _context.ChequeBooks.Where(x => x.IsActive)
                                                    ...
                                                    join t2 in _context.BankBranches on t1.BankAccountInfoId equals t2.BankBranchId 
                                                    join t3 in _context.Banks on t2.BankId  equals t3.BankId
                                                    
                                                    select new ChequeBookModel
                                                    {
                                                        ChequeBookId = t1.ChequeBookId,
                                                        BankAccountInfoId = t2.BankBranchId,
                                                        AccountName = t2.AccountName,
        ...
        public async Task<BankBranchModel> GetBankAccountInfoById(long bankAccountInfoId)
        {
            BankBranchModel sendData = await (from t1 in _context.BankBranches.Where(x => x.BankBranchId == bankAccountInfoId && x.IsActive)
                                                   join t3 in _context.Banks on t1.BankId equals t3.BankId into t3_Join
                                                   from t3 in t3_Join.DefaultIfEmpty()
